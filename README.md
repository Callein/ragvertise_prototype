...
### 🔹 4.2 포트폴리오 랭크 리스트 생성 API (POST /api/rank/ptfo)
  ...
  2. **포트폴리오 검색 및 순위 산출** 
     - **임베딩 및 유사도 계산**  
       - **임베딩 모델 초기화**  
         `SentenceTransformer('all-MiniLM-L6-v2')` 모델을 사용하여 사용자 입력 요약과 태그를 임베딩합니다.
       - **텍스트 유사도 계산 (FAISS 사용)**  
         - 포트폴리오 임베딩 벡터들을 L2 정규화한 후, FAISS의 `IndexFlatIP` (내적 기반 인덱스)를 생성합니다.  
       ...
       - **태그 유사도 계산 (포트폴리오별 FAISS 사용)**  
         - 사용자가 요청에 태그가 포함되어 있다면, 해당 태그들을 임베딩하고 정규화합니다.  
         - 각 포트폴리오의 태그 리스트를 개별적으로 임베딩하여 FAISS 인덱스를 생성한 후,  
           사용자 태그와 포트폴리오 태그 간 최고 유사도(최대 내적)를 계산합니다.  
         - 사전에 설정한 임계값(현재 0.5) 이하의 유사도 값에는 벌점(penalty_factor, 현재 3.0)을 적용하여 조정하고,  
           사용자 요청의 모든 태그에 대해 조정된 유사도의 평균을 산출하여 태그 유사도 점수를 결정합니다.
       - **최종 점수 산출**  
